...
static xQueueHandle robotatQueue;
...
int cpxInternalRouterReceiveRobotat(CPXPacket_t * packet) {
  return xQueueReceive(robotatQueue, packet, M2T(100));
}
...
void cpxInternalRouterRouteIn(const CPXRoutablePacket_t* packet) {
  ...
  switch (packet->route.function) {
    ...
    case CPX_F_CRTP:
      ...
      break;
    case CPX_F_ROBOTAT:
      xQueueSend(robotatQueue, packet, portMAX_DELAY);
      break;
    default:
...
void cpxInternalRouterInit(void) {
  ...
  robotatQueue = xQueueCreate(QUEUE_LENGTH, sizeof(CPXPacket_t));;
}